  ...
  implements ServiceInterface<any, any, ServiceParams, never>
{
  ...
  async create(data: any, params?: ServiceParams): Promise<any> {
    ...
    const req = (params as any)?.koaRequest ?? (params as any)?.req;
    const res = (params as any)?.koaResponse ?? (params as any)?.res;
    if (!req || !res) {
  ...
  private getSessionId(params: ServiceParams | undefined): string | undefined {
  ...
  getEmitFunction(
    ...
    return (message: string, options?: EmitOptions | number) => {
      ...
      const opts =
        typeof options === "number" ? { progress: options } : options || {};
  ...
  private transformToMcpResponse(result: ToolResponse<any>) {
    ...
    const mcpResponse = {
      content: [
        ...(json ? [{ type: "text", text: JSON.stringify(json.result) }] : []),
